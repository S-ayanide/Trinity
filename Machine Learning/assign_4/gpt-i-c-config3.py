"""
GPT Configuration 3: Balanced Reduction (Reduced Context + Layers)
Config: n_embd=128, n_head=4, n_layer=3, block_size=64
Total Parameters: ~0.612M
"""
import torch
...
n_embd = 128
n_head = 4
n_layer = 3  # 3 layers (moderate depth)
dropout = 0.2
# ------------
...
print('\nStarting training...\n')
for iter in tqdm(range(max_iters), desc="Training Config 3"):
...
with open('results/config3_history.json', 'w') as f:
...
with open('results/config3_generated.txt', 'w') as f:
    f.write(generated)

print(f'\nTraining complete! Results saved to results/config3_*')
