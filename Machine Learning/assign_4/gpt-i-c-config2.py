"""
GPT Configuration 2: Shallow Network (Reduced Layers)
Config: n_embd=128, n_head=4, n_layer=2, block_size=256
Total Parameters: ~0.439M
"""
import torch
...
eval_iters = 200
n_embd = 128  # Slightly larger embedding
n_head = 4
n_layer = 2  # Only 2 layers (shallow network)
dropout = 0.2
# ------------
...
print('\nStarting training...\n')
for iter in tqdm(range(max_iters), desc="Training Config 2"):
...
with open('results/config2_history.json', 'w') as f:
...
with open('results/config2_generated.txt', 'w') as f:
    f.write(generated)

print(f'\nTraining complete! Results saved to results/config2_*')
