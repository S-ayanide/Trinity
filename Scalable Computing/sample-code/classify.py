...
def main():
    ...
    with tf.device('/cpu:0'):
        ...
        # Get list of files and sort them alphanumerically
        all_files = [f for f in os.listdir(args.captcha_dir) 
                     if not f.startswith('.') and f.endswith('.png')]
        all_files.sort()  # Sort alphanumerically (0-9, then a-z)
        ...
        for x in all_files:
            ...
            results.append((x, captcha_text))
            print('Classified ' + x)
        
        # Write results to file in sorted order, comma-delimited (no space)
